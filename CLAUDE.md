...
- No external UI or animation libraries — all motion is plain CSS keyframes
...
`src/components/TouchCursor.jsx` — fixed 40×40 semi-transparent circle tracking `mousemove`, scales to 78% on `mousedown`. The native cursor is hidden globally via `* { cursor: none !important }` in `src/index.css`.
